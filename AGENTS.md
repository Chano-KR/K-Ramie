...
- `dist/k-ramie.zip` does not bundle commercial fonts. Run `bash scripts/ensure-fonts.sh` (POSIX / Git Bash / WSL) or `pwsh scripts/ensure-fonts.ps1` (Windows native) to auto-fetch Pretendard and print manual-install paths for the rest. Missing fonts fall back through the system chain and pages still render.
